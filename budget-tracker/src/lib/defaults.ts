import type { Category, CategorizationRule, CategoryPattern } from '../types';

export const DEFAULT_CATEGORIES: Category[] = [
  {
    id: 'groceries',
    name: 'Groceries',
    color: '#10b981',
    patterns: [
      { id: 'gp1', pattern: '(safeway|kroger|whole foods|trader joe|albertsons|publix|wegmans)', priority: 10, enabled: true, description: 'Major grocery chains' },
      { id: 'gp2', pattern: '(grocery|supermarket|market)', priority: 15, enabled: true, description: 'Generic grocery keywords' },
    ],
  },
  {
    id: 'restaurants',
    name: 'Restaurants & Dining',
    color: '#f59e0b',
    patterns: [
      { id: 'rp1', pattern: '(starbucks|coffee|cafe|restaurant|dining)', priority: 10, enabled: true, description: 'Coffee shops and restaurants' },
      { id: 'rp2', pattern: '(mcdonald|burger king|wendy|subway|chipotle|panera)', priority: 10, enabled: true, description: 'Fast food chains' },
      { id: 'rp3', pattern: '(pizza|taco|burrito)', priority: 12, enabled: true, description: 'Common food types' },
    ],
  },
  {
    id: 'transportation',
    name: 'Transportation',
    color: '#3b82f6',
    patterns: [
      { id: 'tp1', pattern: '(uber|lyft|taxi|transit|metro|bart)', priority: 10, enabled: true, description: 'Rideshare and public transit' },
    ],
  },
  {
    id: 'gas',
    name: 'Gas & Fuel',
    color: '#8b5cf6',
    patterns: [
      { id: 'gap1', pattern: '(shell|chevron|exxon|mobil|bp|arco|gas|fuel)', priority: 10, enabled: true, description: 'Gas stations' },
    ],
  },
  {
    id: 'entertainment',
    name: 'Entertainment',
    color: '#ec4899',
    patterns: [
      { id: 'ep1', pattern: '(movie|theater|cinema|concert|ticket)', priority: 10, enabled: true, description: 'Movies and events' },
    ],
  },
  {
    id: 'shopping',
    name: 'Shopping & Retail',
    color: '#ef4444',
    patterns: [
      { id: 'sp1', pattern: '(amazon|target|walmart|costco|best buy)', priority: 10, enabled: true, description: 'Major retailers' },
    ],
  },
  {
    id: 'utilities',
    name: 'Utilities',
    color: '#14b8a6',
    patterns: [
      { id: 'up1', pattern: '(electric|power|water|internet|comcast|at&t|verizon)', priority: 10, enabled: true, description: 'Utilities and internet' },
    ],
  },
  {
    id: 'healthcare',
    name: 'Healthcare',
    color: '#06b6d4',
    patterns: [
      { id: 'hp1', pattern: '(pharmacy|cvs|walgreens|doctor|medical|health)', priority: 10, enabled: true, description: 'Medical and pharmacy' },
    ],
  },
  {
    id: 'insurance',
    name: 'Insurance',
    color: '#6366f1',
    patterns: [],
  },
  {
    id: 'housing',
    name: 'Housing',
    color: '#84cc16',
    patterns: [],
  },
  {
    id: 'subscriptions',
    name: 'Subscriptions',
    color: '#a855f7',
    patterns: [
      { id: 'subp1', pattern: '(netflix|spotify|hulu|disney|amazon prime|apple music)', priority: 10, enabled: true, description: 'Streaming services' },
    ],
  },
  {
    id: 'travel',
    name: 'Travel',
    color: '#0ea5e9',
    patterns: [],
  },
  {
    id: 'services',
    name: 'Services',
    color: '#f97316',
    patterns: [],
  },
  {
    id: 'income',
    name: 'Income/Payments',
    color: '#22c55e',
    patterns: [
      { id: 'ip1', pattern: '(payment|credit|refund)', priority: 10, enabled: true, description: 'Payments and refunds' },
    ],
  },
  {
    id: 'other',
    name: 'Other',
    color: '#64748b',
    patterns: [],
  },
];

export const DEFAULT_RULES: CategorizationRule[] = [
  // Groceries
  { id: 'r1', categoryId: 'groceries', pattern: '(safeway|kroger|whole foods|trader joe|albertsons|publix|wegmans)', priority: 10, enabled: true },
  { id: 'r2', categoryId: 'groceries', pattern: '(grocery|supermarket|market)', priority: 15, enabled: true },

  // Restaurants
  { id: 'r3', categoryId: 'restaurants', pattern: '(starbucks|coffee|cafe|restaurant|dining)', priority: 10, enabled: true },
  { id: 'r4', categoryId: 'restaurants', pattern: '(mcdonald|burger king|wendy|subway|chipotle|panera)', priority: 10, enabled: true },
  { id: 'r5', categoryId: 'restaurants', pattern: '(pizza|taco|burrito)', priority: 12, enabled: true },

  // Transportation
  { id: 'r6', categoryId: 'transportation', pattern: '(uber|lyft|taxi|transit|metro|bart)', priority: 10, enabled: true },

  // Gas
  { id: 'r7', categoryId: 'gas', pattern: '(shell|chevron|exxon|mobil|bp|arco|gas|fuel)', priority: 10, enabled: true },

  // Subscriptions
  { id: 'r8', categoryId: 'subscriptions', pattern: '(netflix|spotify|hulu|disney|amazon prime|apple music)', priority: 10, enabled: true },

  // Utilities
  { id: 'r9', categoryId: 'utilities', pattern: '(electric|power|water|internet|comcast|at&t|verizon)', priority: 10, enabled: true },

  // Shopping
  { id: 'r10', categoryId: 'shopping', pattern: '(amazon|target|walmart|costco|best buy)', priority: 10, enabled: true },

  // Income/Payments
  { id: 'r11', categoryId: 'income', pattern: '(payment|credit|refund)', priority: 10, enabled: true },

  // Healthcare
  { id: 'r12', categoryId: 'healthcare', pattern: '(pharmacy|cvs|walgreens|doctor|medical|health)', priority: 10, enabled: true },

  // Entertainment
  { id: 'r13', categoryId: 'entertainment', pattern: '(movie|theater|cinema|concert|ticket)', priority: 10, enabled: true },
];

export function getDefaultPreferences() {
  return {
    categories: DEFAULT_CATEGORIES,
    rules: DEFAULT_RULES,
    excludedTransactionSignatures: [],
    excludedRepeatedExpensePatterns: [],
    timeWindowFilter: {
      enabled: false,
      startDate: null,
      endDate: null
    },
    version: 1,
    lastModified: new Date(),
  };
}
